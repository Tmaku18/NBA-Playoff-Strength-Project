...
**Two production defaults:** (1) **Standings default** — `config/defaults.yaml` (Phase 3 fine NDCG@16 combo 18): NDCG@16 0.550, Spearman 0.557, playoff_spearman 0.568, listmle_target final_rank; combo path `outputs4/sweeps/phase3_fine_ndcg16_final_rank/combo_0018/config.yaml`. (2) **Playoff-outcome default** — `config/defaults_playoff_outcome.yaml` (Phase 5 combo 2): NDCG@16 0.543, listmle_target playoff_outcome; use `--config config/defaults_playoff_outcome.yaml` for pipeline or inference when you need playoff-outcome–tuned predictions. **Run 21 and Run 22** are baseline full-pipeline runs (not sweep-optimized for a single metric). **Run 21** was the first real success: Model A contributes (attention/contributors), ensemble ranking vs playoff outcome improved. **Run 22** (EOS source: eos_final_rank) achieved NDCG 0.48, Spearman 0.43, playoff Spearman 0.46 — better than early sweeps that optimized only Spearman. Neither run_021 nor run_022 was tuned for NDCG-only or Spearman-only; they used the same default config. **Sweep strategy:** Run separate Optuna sweeps with `--objective spearman`, `--objective ndcg4`, `--objective ndcg16`, `--objective ndcg20`, `--objective playoff_spearman`, or `--objective rank_rmse`, then compare best configs across objectives. **outputs4/** is the production outputs root (run_025+); use `--config config/outputs4_phase1.yaml` for sweeps. **outputs3/** holds baseline and early Phase I sweeps. **Rolling windows sweep (outputs4):** [15, 30] outperformed [10], [10, 30], and [20, 30] on Spearman (best 0.496); use `rolling_windows: [15, 30]` for Phase 2. See **`outputs/ANALYSIS.md`**, **`outputs4/sweeps/SWEEP_PHASE1_ANALYSIS.md`**, and **`outputs2/run_022/RESULTS_AND_OUTPUTS_EXPLAINED.md`** for run comparisons and metric definitions. Sequential sweep commands: [.cursor/plans/outputs4_phase_i_sweeps_2638c514.plan.md](.cursor/plans/outputs4_phase_i_sweeps_2638c514.plan.md) §2.4.
...
All paths under the configured outputs dir. **outputs2/** holds run_020/021; **outputs3/** baseline sweeps; **outputs4/** production sweeps and combo 18 (run_025+, use `config/outputs4_phase1.yaml`). With `inference.run_id: null`, each pipeline run writes to a new folder; evaluation uses the latest run.
