...
## Overview
This project builds a **Multi-Modal Stacking Ensemble** to predict NBA **True Team Strength** using a Deep Set roster model plus XGBoost (Model B). The **ensemble** is Model A + Model B only. **Model C** (Random Forest) is not trained by default (`training.train_model_c: false`); when present (e.g. from older runs), its metrics are kept for analytic comparison only and it is not included in the ensemble. The system targets **future outcomes** and identifies **Sleepers** versus **Paper Tigers** without circular evaluation.
...
**Legacy production defaults (outputs4):** (1) **Standings** — `config/defaults.yaml` (Phase 3 fine NDCG@16 combo 18): listmle_target `final_rank` (playoff standings); combo path `output/outputs4/sweeps/phase3_fine_ndcg16_final_rank/combo_0018/config.yaml`. (2) **Playoff-outcome** — `config/defaults_playoff_outcome.yaml` (Phase 5 combo 2): listmle_target `playoff_outcome`; use `--config config/defaults_playoff_outcome.yaml` for pipeline or inference. **Latest production run (outputs4):** run_026 — ensemble NDCG@10 0.485, Spearman 0.477, playoff Spearman 0.467, NDCG@16 0.527 (eval: eos_final_rank; 141 train / 36 test dates). **ListMLE outcome vs standings:** In **outputs5/** we compared training Model A (ListMLE) on `playoff_outcome` vs `final_rank` (standings); when evaluated on playoff outcome, **standings-trained** ListMLE matched or beat outcome-trained (e.g. ndcg_standing: Spearman 0.529, playoff Spearman 0.531 vs ndcg_outcome 0.491 / 0.475). See **`output/outputs/ANALYSIS.md`** for run_026 details and **§ListMLE outcome vs standings**. **Run 021/022** are baseline full-pipeline runs (not sweep-optimized). **Sweep strategy:** Run separate Optuna sweeps with `--objective spearman`, `--objective ndcg4`, `--objective ndcg16`, `--objective ndcg20`, `--objective playoff_spearman`, or `--objective rank_rmse`, then compare best configs. **outputs4/** is the production outputs root (run_025, run_026); **outputs5/** holds outcome vs standings comparison runs. See **`output/outputs4/sweeps/SWEEP_PHASE1_ANALYSIS.md`**, **`docs/SWEEP_ANALYSIS.md`**, and **`outputs2/run_022/RESULTS_AND_OUTPUTS_EXPLAINED.md`** for sweep and metric definitions.

**Best runs (historical):** **`docs/BEST_METRICS_02-15.md`** documents exact configs and numbers; **outputs6_baseline** phase_1 runs (e.g. Overall Spearman ~0.71–0.75 on phase_1 outcome eval) are superseded by **outputs8_spearman_surrogate** for official best.
...
**Best runs note (outputs6):** The `outputs6/phase_1/outcome/*/eval_report.json` runs include **Overall Spearman ~0.71–0.75** (and Playoff Spearman ~0.69–0.73) on the phase_1 outcome evaluation. See **`docs/BEST_METRICS_02-15.md`** for the exact numbers and the **exact configs** behind each “best_*” folder.
...
- **Report assets:** **outputs/ANALYSIS.md** (current results run_026, split, ListMLE outcome vs standings); All output folders live under **`output/`**. **output/outputs2/** run_020/021/022; **output/outputs3/** baseline and Phase I sweeps; **output/outputs4/** production (run_025, run_026); **output/outputs5/** outcome vs standings; **output/outputs6_baseline/** baseline; **output/outputs7_listmle_sweep/** ListMLE sweep; **output/outputs8_spearman_surrogate/** best (Spearman surrogate); **output/outputs9_listmle_spearman/** ListMLE sweep; **output/outputs10_spearman_surrogate_standing_rank/** Spearman+standing; **output/outputs11_listmle_standing_rank/** ListMLE+standing; **output/outputs12_baseline_standing_rank/** baseline+standing; **output/outputs13_rmse_surrogate/** RMSE surrogate; **output/outputs14_map_run/** MAP run. Each of **output/outputs/** through **output/outputs14/** has a **`MODEL.md`** describing the model and how it differs from the previous output folder. See `output/outputs4/sweeps/SWEEP_PHASE1_ANALYSIS.md` and `docs/SWEEP_ANALYSIS.md` for sweep analysis.
...
2. **Config:** Edit `config/defaults.yaml` if needed (seasons, paths, model params, `build_db.skip_if_exists`, `inference.run_id`). **DB:** `config.paths.db` should point to a DB with playoff data (`playoff_games`, `playoff_team_game_logs`) for sweeps and `--objective playoff_spearman`. From any worktree, set env `NBA_DB_PATH` to that DB path so the sweep uses it (e.g. the main project DB). **Production runs use `output/outputs4/`** (`config.paths.outputs`). The first run in an empty outputs folder can start at `run_001` (or use `inference.run_id_base`); with `inference.run_id: null` (default) runs auto-increment.
3. **Data:**  
...
5. **Inference:** `python -m scripts.6_run_inference` — load DB and models, run Model A/B + meta → outputs dir `<run_id>/predictions.json`, plots. With `inference.run_id: null`, run_id auto-increments (e.g. run_019, run_020 when using `output/outputs2/` and `run_id_base: 19`) so each full pipeline run gets a new folder.
...
7. **Explainability:** `python -m scripts.5b_explain` (uses `config/defaults.yaml`) or `python -m scripts.5b_explain --config path/to/config.yaml` — SHAP on Model B → `shap_summary.png`; attention ablation and Integrated Gradients (Model A) when Captum is installed → `ig_model_a_attributions.txt`. Use `--config` to run explain on a **sweep best combo** (e.g. `output/outputs4/sweeps/phase3_fine_ndcg16_final_rank/combo_0018/config.yaml`).
...
- **Outputs:** All scripts use `config.paths.outputs` (default `outputs4`); scripts 3–6 write models, predictions, and plots there.
...
All paths under the configured outputs dir. **output/outputs/** holds ANALYSIS.md (current results). **output/outputs2/** run_020/021/022; **output/outputs3/** baseline sweeps; **output/outputs4/** production baseline (run_025, run_026, sweeps, combo 18); **output/outputs5/** ListMLE outcome vs standings; **output/outputs6_baseline/** baseline; **output/outputs7_listmle_sweep/** ListMLE sweep; **output/outputs8_spearman_surrogate/** best; **output/outputs9_listmle_spearman/** ListMLE sweep; **output/outputs10_spearman_surrogate_standing_rank/** Spearman+standing; **output/outputs11_listmle_standing_rank/** ListMLE+standing; **output/outputs12_baseline_standing_rank/** baseline+standing; **output/outputs13_rmse_surrogate/** RMSE surrogate; **output/outputs14_map_run/** MAP run. **Each output folder has a `MODEL.md`** that describes the model that produced those results and the difference from the previous output (see [docs/MODEL_LINEUP_AND_NEXT_STEPS.md](docs/MODEL_LINEUP_AND_NEXT_STEPS.md)). With `inference.run_id: null`, each pipeline run writes to a new folder; evaluation uses the latest run.
...
- `output/outputs4/run_026/` — latest production run (EOS `eos_final_rank`): NDCG@10 0.485, Spearman 0.477, playoff Spearman 0.467, NDCG@16 0.527. `output/outputs/ANALYSIS.md` has full run_026 breakdown and ListMLE outcome vs standings. `output/outputs2/run_022/` — baseline; see `outputs2/run_022/RESULTS_AND_OUTPUTS_EXPLAINED.md`.
