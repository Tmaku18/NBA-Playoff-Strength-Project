...
|------|--------|-------------|
| **Baseline** | **outputs6** | Phase 1 outcome runs (ListMLE, playoff_outcome, rolling [15,30]; configs from outputs4). Reference for all comparisons. |
| **Best** | **outputs8** | **Spearman-surrogate** loss, playoff_outcome, 40 Optuna trials. Best Spearman (0.777), playoff_spearman (0.854), rank_mae, rank_rmse, NDCG. Use for production. |
| **MAP run** | **outputs14** | Future MAP branch model; per-game evaluation. Config: `config/outputs14_map_run.yaml`. Compare to baseline (outputs6) and best (outputs8). See [docs/OUTPUTS14_MAP_RUN.md](docs/OUTPUTS14_MAP_RUN.md). |
| **RMSE surrogate** | **outputs13** | **rank_rmse_surrogate** sweep; same setup as outputs8, different loss. Config: `config/outputs13_sweep_rmse_surrogate.yaml`; compare to outputs8. |
| **outputs6-style + standing rank** | **outputs12** | Same as baseline config (ListMLE, final_rank, [15,30]) but **standing rank as input** (stat_dim 22). Single run: `config/outputs4_baseline_standing_rank.yaml` → `outputs12/baseline_standing_rank`. Compare to baseline (outputs6). |
| **ListMLE + standing (sweep)** | **outputs11** | ListMLE sweep with standing rank (Optuna). Compare to **baseline (outputs6)** and outputs10 (Spearman surrogate + standing). |

Each output folder has a **`MODEL.md`** that describes the model that produced those results and the **difference from the previous output** (e.g. `outputs4/MODEL.md`, `outputs8/MODEL.md`). Full lineup and run commands: **[docs/MODEL_LINEUP_AND_NEXT_STEPS.md](docs/MODEL_LINEUP_AND_NEXT_STEPS.md)**. Official best configs: **[docs/OFFICIAL_BEST_CONFIGS_AND_ANALYSIS.md](docs/OFFICIAL_BEST_CONFIGS_AND_ANALYSIS.md)** and **[docs/OUTPUTS8_SWEEP_ANALYSIS_02-17.md](docs/OUTPUTS8_SWEEP_ANALYSIS_02-17.md)**.
...
**Best runs (historical):** **`docs/BEST_METRICS_02-15.md`** documents exact configs and numbers; **outputs6** phase_1 runs (e.g. Overall Spearman ~0.71–0.75 on phase_1 outcome eval) are superseded by **outputs8** for official best.
...
- **Report assets:** **outputs/ANALYSIS.md** (current results run_026, split, ListMLE outcome vs standings); **outputs2/** run_020/021/022; **outputs3/** baseline and Phase I sweeps; **outputs4/** production (run_025, run_026); **outputs5/** outcome vs standings; **outputs6/** phase_1 outcome; **outputs7/** ListMLE sweep; **outputs8/** best (Spearman surrogate); **outputs9/** ListMLE sweep; **outputs10/** Spearman surrogate + standing rank; **outputs13/** RMSE surrogate sweep; **outputs14/** MAP run (future). Each of **outputs/** through **outputs14/** has a **`MODEL.md`** describing the model and how it differs from the previous output folder. See `outputs4/sweeps/SWEEP_PHASE1_ANALYSIS.md` and `docs/SWEEP_ANALYSIS.md` for sweep analysis.
...
2. **Config:** Edit `config/defaults.yaml` if needed (seasons, paths, model params, `build_db.skip_if_exists`, `inference.run_id`). **DB:** `config.paths.db` should point to a DB with playoff data (`playoff_games`, `playoff_team_game_logs`) for sweeps and `--objective playoff_spearman`. From any worktree, set env `NBA_DB_PATH` to that DB path so the sweep uses it (e.g. the main project DB). **Production runs use `outputs4/`** (`config.paths.outputs`). The first run in an empty outputs folder can start at `run_001` (or use `inference.run_id_base`); with `inference.run_id: null` (default) runs auto-increment.
3. **Data:**  
...
7. **Explainability:** `python -m scripts.5b_explain` (uses `config/defaults.yaml`) or `python -m scripts.5b_explain --config path/to/config.yaml` — SHAP on Model B → `shap_summary.png`; attention ablation and Integrated Gradients (Model A) when Captum is installed → `ig_model_a_attributions.txt`. Use `--config` to run explain on a **sweep best combo** (e.g. `outputs4/sweeps/phase3_fine_ndcg16_final_rank/combo_0018/config.yaml`).
...
All paths under the configured outputs dir. **outputs/** holds ANALYSIS.md (current results). **outputs2/** run_020/021/022; **outputs3/** baseline sweeps; **outputs4/** production baseline (run_025, run_026, sweeps, combo 18); **outputs5/** ListMLE outcome vs standings; **outputs6/** phase_1 outcome; **outputs7/** ListMLE sweep; **outputs8/** best (Spearman surrogate); **outputs9/** ListMLE sweep; **outputs10/** Spearman surrogate + standing rank; **outputs13/** RMSE surrogate sweep; **outputs14/** MAP run. **Each output folder has a `MODEL.md`** that describes the model that produced those results and the difference from the previous output (see [docs/MODEL_LINEUP_AND_NEXT_STEPS.md](docs/MODEL_LINEUP_AND_NEXT_STEPS.md)). With `inference.run_id: null`, each pipeline run writes to a new folder; evaluation uses the latest run.
...
- `outputs4/run_026/` — latest production run (EOS `eos_final_rank`): NDCG@10 0.485, Spearman 0.477, playoff Spearman 0.467, NDCG@16 0.527. `outputs/ANALYSIS.md` has full run_026 breakdown and ListMLE outcome vs standings. `outputs2/run_022/` — baseline; see `outputs2/run_022/RESULTS_AND_OUTPUTS_EXPLAINED.md`.
