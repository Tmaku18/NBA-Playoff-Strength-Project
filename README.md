...
- **True Strength:** Model A produces a latent **Z** (penultimate layer); the **output** `ensemble_score` is the **ensemble** score (RidgeCV blend of A + XGB only) mapped to percentile (0–1 and 0–100). Model C (LR) is for diagnostics/comparison only.
...
All paths under the configured outputs dir. **outputs/** holds ANALYSIS.md (current results). **outputs2/** run_020/021/022; **outputs3/** baseline sweeps; **outputs4/** production (run_025, run_026, sweeps, combo 18); **outputs5/** ListMLE outcome vs standings comparison (ndcg_outcome, ndcg_standing, spearman_outcome, spearman_standing). With `inference.run_id: null`, each pipeline run writes to a new folder; evaluation uses the latest run.
