...
## 5. Conference-Specific vs. League-Wide (East vs West)

All evaluation runs report **East vs West** metrics so we can compare ranking quality within each conference.

**Where to find East/West metrics:** In each run folder, `eval_report.json` (and per-season `eval_report_<season>.json`) contains **`test_metrics_by_conference`** with keys `"E"` (East) and `"W"` (West). For each conference you get:

- **ndcg**, **spearman** — Within-conference ranking (relevance = derived from EOS global rank so best team in conference = 1).
- **rank_mae_ensemble_pred_vs_playoff_outcome_rank**, **rank_rmse_ensemble_pred_vs_playoff_outcome_rank** — Ensemble rank error vs playoff outcome in that conference.
- Same **rank_mae** / **rank_rmse** for **model_a**, **xgb**, **rf** and for **wl_record_standings** (baseline).

**Why East and West can differ:** Model A is trained on conference-date lists (E and W are separate lists); Model B is team-level, so E/W is only an evaluation split. Differences can reflect roster depth, strength spread, or how well team-context features correlate with outcome in each conference. For the caveat on using *global* rank within one conference, see `docs/CHECKPOINT_PROJECT_REPORT_02-01.md` §3.5 and `docs/MODEL_A_VS_B_02-15.md` §7.
