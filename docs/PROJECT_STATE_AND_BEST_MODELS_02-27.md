...
- **Next**: stabilize Model A (seed averaging / more lists), revisit meta scale handling (rank-transform meta inputs), run the top-weighted variant and the `--phase flags` ablation, and fix a late-season snapshot bug in `build_team_context_as_of_dates` (corrupted eFG/pace on a 2024-04-14 inference snapshot).
