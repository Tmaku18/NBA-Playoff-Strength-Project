...
### 4a. Multi-temperature Model A (optional)

When `model_a.attention.multi_temp_enabled` is true, Model A is run at 6 temperatures (1, 3, 5, 6, 8, 10) and scores are aggregated with confidence-weighted combination. Temp 5 (starting lineup) receives the highest base weight; agreement across temperatures yields high confidence \(c_A\). Availability modulation (starter availability, minutes_eff) implements injury-aware ranking (H1/H2). See `docs/ATTENTION_TEMPERATURE_SEMANTICS_02-12.md`.
...
2. **Script 3:** Train Model A (Deep Set + ListMLE); produce OOF and optionally `conf_a` (single-temp from attention or multi-temp from agreement); save `best_deep_set.pt` and `oof_model_a.parquet`.
3. **Script 4:** Train Model B (XGB) and Model C (RF); produce OOF and optionally `conf_xgb`; save `oof_model_b.parquet`, `xgb_model.joblib`, `rf_model.joblib`.
...
## 6. Precision and smoothing

Baseline W/L standings can outperform models on rank MAE/RMSE vs playoff outcome. We are exploring reduced smoothing (rolling windows, temperatures, percentile) and other precision levers. See `docs/PRECISION_AND_SMOOTHING_02-12.md` for smoothing sources, proposed experiments, and recommendation order.

---

## 7. References and docs
...
- **Model A vs B comparison:** `docs/MODEL_A_VS_B_02-15.md` — inputs, training, inference, strengths/weaknesses, performance; see `docs/BEST_METRICS_02-15.md` for run tables.
