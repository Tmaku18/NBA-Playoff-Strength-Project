...
def _uncertainty_metrics_from_teams(teams: list) -> dict:
    """Compute interval coverage/width for rank predictions when interval fields exist."""

    def _accum_for_interval(get_low_high) -> dict:
        covered = 0
        total = 0
        widths: list[float] = []
        for t in teams:
            ...
            total += 1
            widths.append(float(high_i - low_i))
            if low_i <= int(act) <= high_i:
                covered += 1
        if total == 0:
            return {}
        return {
    ...
    out: dict[str, Any] = {}
    out["ensemble"] = _accum_for_interval(
        lambda t: (
            t.get("prediction", {}).get("predicted_strength_low"),
            t.get("prediction", {}).get("predicted_strength_high"),
        )
    )
    out["model_a"] = _accum_for_interval(
        lambda t: (
            t.get("ensemble_diagnostics", {}).get("model_a_rank_low"),
            t.get("ensemble_diagnostics", {}).get("model_a_rank_high"),
        )
    )
    out["model_b"] = _accum_for_interval(
        lambda t: (
            t.get("ensemble_diagnostics", {}).get("model_b_rank_low"),
            t.get("ensemble_diagnostics", {}).get("model_b_rank_high"),
        )
    )
    out["model_c"] = _accum_for_interval(
        lambda t: (
            t.get("ensemble_diagnostics", {}).get("model_c_rank_low"),
            t.get("ensemble_diagnostics", {}).get("model_c_rank_high"),
        )
    )
    ...
    names: set[str] = set()
    for t in teams:
        d = t.get("ensemble_diagnostics", {}).get("extra_model_ranks") or {}
        if isinstance(d, dict):
            for k in d.keys():
                names.add(str(k))
    for name in sorted(names):
        extra[name] = _accum_for_interval(
            lambda t, nm=name: (
                (t.get("ensemble_diagnostics", {}).get("extra_model_ranks") or {}).get(nm, {}).get("rank_low"),
                (t.get("ensemble_diagnostics", {}).get("extra_model_ranks") or {}).get(nm, {}).get("rank_high"),
            )
        )
    if extra:
        out["extra_models"] = extra
...
def main():
    ...
    try:
        ...
        has_train = (run_dir / "train_predictions.json").exists()
        has_val = (run_dir / "val_predictions.json").exists()
        report["notes"]["eval_on"] = "test" + ("+train" if has_train else "") + ("+val" if has_val else "")
    ...
    if not by_season and pred_path.exists():
        ...
        if primary_teams:
            ...
            report["uncertainty_metrics"] = _uncertainty_metrics_from_teams(primary_teams)
    ...
    report["notes"]["train_val_test_accuracy"] = (
        "train_metrics_* / val_metrics_* / test_metrics_*: ranking metrics (Spearman, NDCG, rank_mae, rank_rmse, playoff_metrics) "
        "for ensemble, model_a, model_b, model_c. Enable with inference.also_train_predictions and inference.also_validation_predictions."
    )
    ...
    # Validation metrics (if val_predictions.json exists)
    val_pred_path = run_dir / "val_predictions.json"
    if val_pred_path.exists():
        with open(val_pred_path, "r", encoding="utf-8") as f:
            val_data = json.load(f)
        val_teams = val_data.get("teams", [])
        if val_teams:
            report["val_metrics_ensemble"] = _compute_metrics(val_teams)
            by_model_v, _, _ = _teams_to_arrays_by_model(val_teams)
            report["val_metrics_model_a"] = _compute_metrics_from_arrays(by_model_v["model_a"][0], by_model_v["model_a"][1], by_model_v["model_a"][2]) if "model_a" in by_model_v else {}
            report["val_metrics_model_b"] = _compute_metrics_from_arrays(by_model_v["xgb"][0], by_model_v["xgb"][1], by_model_v["xgb"][2]) if "xgb" in by_model_v else {}
            report["val_metrics_model_c"] = _compute_metrics_from_arrays(by_model_v["rf"][0], by_model_v["rf"][1], by_model_v["rf"][2]) if "rf" in by_model_v else {}
            report["val_metrics_by_conference"] = _metrics_by_conference(val_teams)
            report["notes"]["val_eos_rank_source"] = val_data.get("eos_rank_source", "standings")
