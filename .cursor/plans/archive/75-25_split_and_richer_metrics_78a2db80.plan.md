...
1. **75/25 split:** Hold out 25% of data as a **test set** (preferably by **season** or by **date cutoff**) so time order is preserved; train Model A, Model B, and stacking only on the **75% train** portion. Run inference and evaluation on the **test** portion so reported metrics are unbiased.
...
- **Internal validation:** Model A uses `early_stopping_val_frac` from config (currently **0.25**); Model B uses **last 20% of dates** for early stopping in final fit. Neither is a true held-out test set.
...
- **Config:** Add to [config/defaults.yaml](config/defaults.yaml) under `training`:
  - `train_seasons: ["2015-16", "2016-17", ...]` (recommended)
  - `test_seasons: ["2023-24", "2024-25"]` (recommended)
  - `train_test_cutoff: "2023-04-15"` (optional date cutoff alternative)
  - `train_frac: 0.75` (fallback if seasons/cutoff not provided)
- **Split rule (recommended):** Season-based holdout using `seasons` ranges in config: lists whose `as_of_date` falls within `train_seasons` go to train; lists in `test_seasons` go to test. This keeps NBA temporal boundaries intact.
- **Split rule (fallback):** Sort lists by `(as_of_date, conference)`; take first 75% of **unique dates** as train, last 25% as test. All lists whose `as_of_date` is in the train dates = train; rest = test. (Using dates keeps time order; no future leakage.)
- **Persistence:** Write `outputs/split_info.json` (or similar) when building the split, containing e.g.:
  ...
  - `train_frac`, `n_train_lists`, `n_test_lists`, `split_mode` (seasons/cutoff/fallback)
...
- **Option B:** Shared helper (e.g. in `src/training/build_lists.py` or `src/utils/split.py`) that, given `lists` and config, returns `(train_lists, test_lists, split_info)` and optionally writes `split_info.json`. Include a helper like `filter_lists_by_seasons()` that maps `as_of_date` into `seasons` ranges and selects train/test lists by season.
...
- After `build_lists` and filtering to `valid_lists`:
  - **If `train_seasons`/`test_seasons` set:** filter by season ranges in config (`seasons`) to produce `train_lists` and `test_lists`.
  - **Else if `train_test_cutoff` set:** train = dates ≤ cutoff, test = dates > cutoff.
  - **Else:** date-based 75/25 fallback (`train_frac`).
  - Write `outputs/split_info.json` with `train_dates`, `test_dates`, and counts (and which mode was used: seasons/cutoff/fallback).
  - Use **train_lists** only for OOF (time-based folds within train) and final model (subsample from train_lists only).
  - Keep internal early-stopping split (`early_stopping_val_frac`, currently 0.25) **within train** only.

---

## 4. Script 4 (Model B)

- After `build_lists`, read `outputs/split_info.json` (or recompute split with same rules if missing).
  ...
  - Keep Model B’s internal early-stopping split **within train** only.
  - Optionally keep `test_df` to support separate evaluation features if needed later.
- Same train/test definition as Model A.
...
- **Summary in report:** Add `split_info` (or key fields): `train_frac`, `n_train_dates`, `n_test_dates`, `split_mode`, `eval_on` (e.g. "test" or "test+train"), so the report is self-describing.
...
flowchart LR
  subgraph split [Split]
    Lists[build_lists]
    SplitLogic["season_based_or_cutoff_or_75_25"]
...
| Item                                                                                                             | Change                                                                                                                                                                              |
| ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [config/defaults.yaml](config/defaults.yaml)                                                                     | Add `training.train_seasons`, `training.test_seasons`, `training.train_test_cutoff`, and `training.train_frac`.                                                                      |
| [scripts/3_train_model_a.py](scripts/3_train_model_a.py)                                                         | Compute split via seasons/cutoff/fallback; write `split_info.json`; use only train lists for OOF and final model.                                                                    |
...
- **Season config mismatch:** If `train_seasons`/`test_seasons` overlap or are missing in `seasons` config, fail fast with a clear error and fall back to date-based split only if explicitly allowed.
...
```json
{
  "split_info": { "train_frac": 0.75, "n_train_dates": 150, "n_test_dates": 50, "split_mode": "seasons" },
